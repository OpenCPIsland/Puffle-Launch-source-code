...
public class UIScrollList : MonoBehaviour, IEZDragDrop, IUIObject
{
	...
	{
		clientClippingRect.FromPoints(new Vector3((0f - viewableAreaActual.x) * 0.5f, viewableAreaActual.y * 0.5f), new Vector3(viewableAreaActual.x * 0.5f, viewableAreaActual.y * 0.5f), new Vector3((0f - viewableAreaActual.x) * 0.5f, (0f - viewableAreaActual.y) * 0.5f));
	...
	protected void PositionNewItems()
	{
		...
		for (int i = 0; i < newItems.Count; i++)
		{
			...
			iUIListObject2.transform.localPosition = new Vector3(x, y);
		}
	...
	protected void UpdateContentExtents(float change)
	{
		...
		{
			scrollPosAnim = AnimateRotation.Do(base.gameObject, EZAnimation.ANIM_MODE.By, Vector3.zero, ScrollPosInterpolator, positionEaseDuration, positionEaseDelay, null, OnPosEasingDone);
	...
	protected void PositionHorizontally(bool updateExtents)
	{
		...
		if (direction == DIRECTION.TtoB_LtoR)
		{
			...
			for (int i = 0; i < items.Count; i++)
			{
				...
				vector = new Vector3(num - items[i].TopLeftEdge.x, itemAlignmentDel(items[i]));
				if (doItemEasing)
				{
		...
		for (int j = 0; j < items.Count; j++)
		{
			...
			vector = new Vector3(num - items[j].BottomRightEdge.x, itemAlignmentDel(items[j]));
			if (doItemEasing)
			{
	...
	protected void PositionVertically(bool updateExtents)
	{
		...
		if (direction == DIRECTION.TtoB_LtoR)
		{
			...
			for (int i = 0; i < items.Count; i++)
			{
				...
				vector = new Vector3(itemAlignmentDel(items[i]), num - items[i].TopLeftEdge.y);
				if (doItemEasing)
				{
					if (newItems.Contains(items[i]))
					{
		...
		for (int j = 0; j < items.Count; j++)
		{
			...
			vector = new Vector3(itemAlignmentDel(items[j]), num - items[j].BottomRightEdge.y);
			if (doItemEasing)
			{
				if (newItems.Contains(items[j]))
				{
	...
	public bool RequestContainership(IUIContainer cont)
	{
		...
		while (parent != null)
		{
			...
			if (parent.gameObject.GetComponent(typeof(IUIContainer)) != null)
			{
				return false;
			}
