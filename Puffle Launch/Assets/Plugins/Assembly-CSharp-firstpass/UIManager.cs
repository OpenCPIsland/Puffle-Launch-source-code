...
public class UIManager : MonoBehaviour
{
	...
	private Material runtimeDefaultFontMaterial;
	...
	{
		if (runtimeDefaultFontMaterial != null)
		{
			UnityEngine.Object.Destroy(runtimeDefaultFontMaterial);
			runtimeDefaultFontMaterial = null;
		}
		s_Instance = null;
	}
		...
		else
		{
			...
			case POINTER_INFO.INPUT_EVENT.DRAG:
				if (curPtr.evt == POINTER_INFO.INPUT_EVENT.RELEASE || curPtr.evt == POINTER_INFO.INPUT_EVENT.TAP)
				{
					...
					{
						tempObj = hit.collider.gameObject.GetComponent(typeof(IUIObject)) as IUIObject;
						curPtr.hitInfo = hit;
						if (tempObj != null)
						{
			...
			case POINTER_INFO.INPUT_EVENT.PRESS:
				if (Physics.Raycast(curPtr.ray, out hit, curPtr.rayDepth, curPtr.layerMask))
				{
					...
					if (curPtr.targetObj != null)
					{
						...
						if (curPtr.targetObj != focusObj && curPtr.type == POINTER_INFO.POINTER_TYPE.RAY == focusWithRay)
						{
	...
	protected void EnsureDefaultFontReferences()
	{
		if (defaultFont == null)
		{
			defaultFont = Resources.Load("font_bmp/CPWonderboy", typeof(TextAsset)) as TextAsset;
		}
		if (defaultFontMaterial == null)
		{
			Material material = Resources.Load("font_bmp/CPWonderboy", typeof(Material)) as Material;
			if (material != null)
			{
				runtimeDefaultFontMaterial = new Material(material);
				runtimeDefaultFontMaterial.name = material.name + " Runtime";
				if (runtimeDefaultFontMaterial.HasProperty("_Color"))
				{
					runtimeDefaultFontMaterial.SetColor("_Color", Color.white);
				}
				defaultFontMaterial = runtimeDefaultFontMaterial;
			}
		}
	}
}
